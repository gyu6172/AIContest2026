...
def generate_cot_reasoning(task: str, op: str, choice: int, candidate: dict, value: str) -> str:
    """학습 데이터용: 정답 기반 1줄 추론 문장 생성 (CoT SFT)."""
    ...
    desc = f"Element {choice}" + (f" ({tag}: {label[:40]})" if label else f" ({tag})")
    if op == "CLICK":
        return f"The task requires a click action. {desc} matches the target."
    elif op == "TYPE":
        return f"The task requires typing '{value[:30]}'. {desc} is the input field."
    else:  # SELECT
        return f"The task requires selecting '{value[:30]}'. {desc} has matching options."
