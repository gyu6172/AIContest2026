...
def process_attrs_structured(attrs):
    """
    attrs 문자열의 key=value 구조는 유지하면서 value만 전처리합니다.
    예: label=Manager name | name=manager -> label=manager | name=manager
    """
    if not isinstance(attrs, str):
        return ""

    processed_attrs = []
    for key, val in ATTR_RE.findall(attrs):
        key = key.strip().lower()
        val_clean = clean_text_advanced(val.strip())
        if val_clean:
            processed_attrs.append(f"{key}={val_clean}")

    return " | ".join(processed_attrs)


def process_candidates_structured(candidates_json):
    """
    candidate_elements의 JSON 배열 구조를 유지하면서 각 후보 필드를 전처리합니다.
    candidate_id/tag/text/attrs 키를 보존해야 target_id 학습과 제출 생성에 사용할 수 있습니다.
    """
    try:
    ...
    for c in cands:
        ...
        # 후보 ID는 target_id와 매칭되는 핵심 값이므로 원본 그대로 보존합니다.
        candidate_id = c.get("candidate_id") or ""

        # tag는 op(TYPE/SELECT/CLICK) 추론에 쓰이므로 구조와 값을 유지합니다.
        tag = str(c.get("tag") or "unknown").strip().lower()

        structured_candidates.append(
            {
                "candidate_id": candidate_id,
                "tag": tag,
                "text": clean_text_advanced(c.get("text")),
                "attrs": process_attrs_structured(c.get("attrs")),
            }
        )

    return json.dumps(structured_candidates, ensure_ascii=False)


def main():
    print("--- [Step 0] 데이터 로딩 ---")
    train_path = DATA_DIR / "test.csv"
    ...
    print("--- [Step 3] candidate_elements JSON 구조 유지 전처리 ---")
    candidate_processed = [
        process_candidates_structured(x) for x in df["candidate_elements"]
    ]
    ...
    preprocessed_df = pd.DataFrame(
        {
            ...
            "candidate_elements": candidate_processed,
        }
    )

    output_path = SAVE_DIR / "preprocessed_test_v5.csv"
    ...
    new_cand_len = preprocessed_df["candidate_elements"].str.len().mean()

    print(f"\n--- [최종 결과 보고] ---")
    print(f"1. 파일 저장 완료: {output_path}")
    print(
        f"2. HTML 최적화: {orig_html_len:.1f}자 -> {new_html_len:.1f}자 (약 {(1 - new_html_len/orig_html_len)*100:.1f}% 감소)"
    )
