...
def fit_ranker(
    ...
    ranker = lgb.LGBMRanker(
        ...
        bagging_fraction=config.get("bagging_fraction", 0.9),
        bagging_freq=1,
        lambda_l2=1.0,
        label_gain=[0, 1],
...
def main():
    ...
    for config in ENSEMBLE_CONFIGS:
        print(f"  - {config['name']}")
        ranker = fit_ranker(X_trn_model, y_trn, g_trn, X_val_model, y_val, g_val, config=config)
        scores = ranker.predict(X_val_model, num_iteration=ranker.best_iteration_)
        rankers.append(ranker)
        val_score_parts.append(scores)
        single_metrics = evaluate_predictions(predict_from_scores(scores, g_val, meta_val))
        model_summaries.append(
            {
                "name": config["name"],
                "best_iteration": int(ranker.best_iteration_ or 0),
                "target_id_acc": single_metrics["target_id_acc"],
                "all_match_acc": single_metrics["all_match_acc"],
            }
        )
            ...
            pd.DataFrame(
                {
                    ...
                    f"{config['name']}_gain": ranker.booster_.feature_importance(importance_type="gain"),
                    f"{config['name']}_split": ranker.booster_.feature_importance(importance_type="split"),
                }
            )
        )
    ...
    metrics["ensemble_size"] = len(ENSEMBLE_CONFIGS)
    ...
    for part in importance_parts[1:]:
        importance = importance.merge(part, on="feature", how="left")
    gain_cols = [c for c in importance.columns if c.endswith("_gain")]
    split_cols = [c for c in importance.columns if c.endswith("_split")]
    importance["gain"] = importance[gain_cols].mean(axis=1)
    importance["split"] = importance[split_cols].mean(axis=1)
    ...
    final_rankers = []
    final_iterations = []
    for config, ranker in zip(ENSEMBLE_CONFIGS, rankers):
        final_iters = int((ranker.best_iteration_ or 500) * 1.15)
        final_iterations.append(final_iters)
        print(f"  - {config['name']} best_iteration={ranker.best_iteration_} final_iters={final_iters}")
        final_rankers.append(fit_ranker(X_all_model, y_all, g_all, n_estimators=final_iters, config=config))

    bundle = {
        "models": final_rankers,
        "ensemble_configs": ENSEMBLE_CONFIGS,
        "holdout_model_summaries": model_summaries,
    ...
    if TEST_PATH.exists() and RAW_TEST_PATH.exists():
        ...
        if len(X_test_model):
            test_scores = np.mean([model.predict(X_test_model) for model in final_rankers], axis=0)
        else:
            test_scores = np.zeros(0)
        submission = save_submission(test, test_scores, g_test, meta_test, SCRIPT_DIR / "submission_lgbm_ranker_v3.csv")
        submission.to_csv(OUTPUT_DIR / "submission_lgbm_ranker_v3.csv", index=False, lineterminator="\n")
